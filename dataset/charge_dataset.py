...
def charge_prediction():
    ...
    # 查询生成的数据并导出为 JSON 文件
    cursor.execute("SELECT * FROM sequential_output_charge")
    ...
    # 写入 JSON 文件
    with open('./data/sequential_output_charge.json', 'w', encoding='utf-8') as json_file:
        json.dump(data, json_file, ensure_ascii=False, indent=4)

    print("数据已导出到 sequential_output_charge.json 文件中。")
