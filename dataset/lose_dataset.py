...
def lose_prediction():
    ...
    for i in range(30):  # 假设生成 30 天的数据
        seq_time_start = start_date + timedelta(days=i)
        seq_time_end = seq_time_start + timedelta(days=6)
    ...
    # 查询生成的数据并导出为 JSON 文件
    cursor.execute("SELECT * FROM sequential_output_lose")
    ...
    for row in rows:
        # 将日期字段转换为字符串
        row['train_start_date'] = row['train_start_date'].isoformat()
        row['train_end_date'] = row['train_end_date'].isoformat()
        row['test_start_date'] = row['test_start_date'].isoformat()
        row['test_end_date'] = row['test_end_date'].isoformat()
        row['part_date'] = row['part_date'].isoformat()
        data.append(row)

    # 写入 JSON 文件
    with open('./data/sequential_output_lose.json', 'w', encoding='utf-8') as json_file:
        json.dump(data, json_file, ensure_ascii=False, indent=4)

    print("数据已导出到 sequential_output_lose.json 文件中。")
